...
async def main():
    ...
    parser.add_argument("--user_id", type=int, default=os.getenv("MY_TELEGRAM_USER_ID"), help="Telegram user ID")
    ...
    parser.add_argument(
        ...
        type=str,
        default="summary.md",
        help="Output file for query results (default: sample_raw_garmin_data.json)",
    )
    ...
    markdown = await garmin_service.generate_markdown_report(
    ...
    with open(output_path, "w") as f:
        f.write(markdown)
        logger.info(f"Markdown report generated for user {args.user_id} from {start_date} to {end_date}")
